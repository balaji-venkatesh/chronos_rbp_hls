...
for line in configs:
    ...
    if (line.find("N_TASK_TYPES")>=0):
        n_task_types = int(line.split()[1])
    if (line.find("mt_core")>=0):
        ...
        if (line.find("terminate")>=0):
            task_type = n_task_types - 4
        if (line.find("all_tasks")>=0):
            task_type = n_task_types - 3
...
if (is_hls):
    out_config.write('`define HLS\n\n')
if (n_task_types != 16):
    out_config.write('`define CUSTOM_TASK_TYPES\n\n')
    out_config.write('parameter N_TASK_TYPES = ' + str(n_task_types) + ';\n\n')
...
core_inst = """ 
        ...
        .ap_idle         (ap_idle),
        .ap_ready        (ap_ready),
        .task_in         ({8'b0, task_in.args, task_in.ttype, task_in.object, task_in.ts}),
